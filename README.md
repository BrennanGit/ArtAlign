...
In reference or layer transform mode, tap other layers' transform icons to move, resize, and rotate them together. Selected icons turn red with a highlighted background; the active layer drives the handles, and selecting the Reference group includes its reference items. Tap a selected icon again to remove it; another selected layer becomes active when needed, and removing the last one exits transform mode. Captures and guides can also be transformed from their row icons. The resulting positions are saved and can be undone together.
