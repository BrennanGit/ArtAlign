...
In reference or layer transform mode, check the boxes beside other layers to move, resize, and rotate them together. The active layer drives the handles; a checked Reference group includes its reference items. Captures and guides can also be transformed from their edit menus. Selection clears when leaving transform mode, while the resulting positions are saved and can be undone together.
